 ...
 * while the config projection is stalled. Shared server keys (see
 * `SHARED_SERVER_SETTING_KEYS`) are written to every eligible sync target,
 * not only the selected target, so a user preference does not silently drift
 * between machines. Client keys go through client persistence.
 */
        ...
        if (Object.keys(sharedPatch).length > 0) {
          ...
          for (const targetId of targets) {
            ...
            void persistServerSettings({
              ...
              if (result._tag === "Success") {
                setAckedServerSettings(targetId, result.value);
              }
            });
          }
          if (!wroteToTarget) {

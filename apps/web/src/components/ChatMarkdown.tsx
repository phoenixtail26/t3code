      ...
      try {
        const clicked = await api.contextMenu.show(
          [
            ...
            ...(isDirectory
              ? ([
                  { id: "open-directory", label: openInFileExplorerLabel(navigator.platform) },
                ] as const)
              : onOpen
          ...
          hasPrimaryAction ? (
            <a
              ...
              onClick={(event) => {
                event.preventDefault();
                event.stopPropagation();
                // A directory has nothing to preview or edit — open it in the OS
...
function useChatMarkdownState({
  ...
  const inlineCodeFileLinkMetaByText = useMemo(() => {
    ...
    for (const span of extractInlineCodeSpans(text)) {
      ...
      const meta = resolveInlineFileMentionMeta(span, cwd, imageBaseDir ?? cwd);
      if (meta) {
  ...
  code: function MarkdownCode({ node, children, className, ...props }) {
    ...
    if (node?.properties?.dataInlineCode != null) {
      ...
      const fileLinkMeta =
        ...
        resolveInlineFileMentionMeta(codeText, cwd, imageBaseDir ?? cwd);
      if (fileLinkMeta) {
        return fileLinkChip(
          fileLinkMeta,
          `\`${codeText}\``,
